/*******************************************************************************
 * PolarFire SoC Discovery Kit - Step 5 Diagnostic Suite
...
/* Reads GEM Hardware MIB registers directly to avoid HAL struct version errors */
...
static void send_ping_request(uint8_t ip1, uint8_t ip2, uint8_t ip3, uint8_t ip4) {
    ...
    /* Step 1: Send ARP Request to resolve Target IP -> Target MAC */
    uint8_t arp_frame[60] = {0};
    memset(&arp_frame[0], 0xFF, 6); /* Broadcast L2 Destination */
    memcpy(&arp_frame[6], g_test_mac->mac_addr, 6);
    arp_frame[12] = 0x08; arp_frame[13] = 0x06; /* EtherType: ARP */
    arp_frame[14] = 0x00; arp_frame[15] = 0x01; /* HW Type: Ethernet */
    arp_frame[16] = 0x08; arp_frame[17] = 0x00; /* Proto: IPv4 */
    arp_frame[18] = 0x06; arp_frame[19] = 0x04;
    arp_frame[20] = 0x00; arp_frame[21] = 0x01; /* Opcode: ARP Request */
    ...
    while (arp_timeout--) {
        if (PACKET_DONE == g_capture) {
            ...
            if (g_packet_length >= 42) {
                ...
                if (ethertype == 0x0806 && opcode == 2) { /* ARP Reply */
    ...
    frame[22] = 64;   frame[23] = 1;    /* Protocol: ICMP */
    ...
    frame[34] = 8; /* ICMP Type 8: Echo Request */
    frame[35] = 0;
        ...
        if (PACKET_DONE == g_capture) {
            ...
            if (rx_len >= 42) {
                ...
                if (ethertype == 0x0800 && protocol == 1) {
                    ...
                    if (icmp_type == 0) { /* ICMP Type 0: Echo Reply */
