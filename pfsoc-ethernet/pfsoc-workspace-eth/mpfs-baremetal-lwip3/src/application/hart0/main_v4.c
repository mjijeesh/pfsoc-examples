/*******************************************************************************
 * PolarFire SoC Discovery Kit - Bare-Metal lwIP Engine with Monitor Toggle
 * Target: MPFS GEM0 + VSC8221 SGMII PHY (MDIO Address 11)
 * Static IP: 192.168.2.207 | DHCP | ICMP Ping | HTTP Server | CLI Monitor
 *******************************************************************************/
...
static volatile bool g_debug_monitor = false; /* Default: OFF to keep CLI clean */
...
static void print_status(void) {
    ...
    /* Convert IP addresses into distinct local buffers to prevent static buffer overwrites */
...
void mac_task(void *pvParameters) {
    ...
    PRINT_STRING("\r\n=======================================================\r\n");
    PRINT_STRING(" PolarFire SoC - Enhanced Bare-Metal lwIP Engine\r\n");
    PRINT_STRING("=======================================================\r\n");
    __enable_irq();
    ...
    while (1) {
        ...
        /* Thread-Safe Dequeue & Packet Inspection Debugger */
        while (g_rx_q_head != g_rx_q_tail) {
            ...
            if (p != NULL) {
                ...
                /* Inbound Traffic Protocol Inspector (Guarded by g_debug_monitor) */
                if (g_debug_monitor && len >= 14) {
                    ...
                    if (ethertype == 0x0806 && len >= 28) { /* ARP */
                        ...
                        if (proto == 1 && len >= 38 && data[34] == 8) { /* ICMP Echo Req */
                            ...
                            if (dport == 80 && (data[47] & 0x02)) { /* SYN to Port 80 */
                                sprintf(dbg_buf, "\r\n[DEBUG] HTTP Connection Request (Port 80) from %u.%u.%u.%u\r\neth-cli> ",
                                        data[26], data[27], data[28], data[29]);
                                PRINT_STRING(dbg_buf);
                            }
                        }
        ...
        /* Service Interactive CLI Commands */
