/*******************************************************************************
 * PolarFire SoC Discovery Kit - Thread-Safe Bare-Metal lwIP Engine
...
/* Raw Frame Container - Keeps ISR completely free of lwIP APIs */
typedef struct {
...
/* Fast Hardware ISR: Copies RAW Bytes Only — Zero lwIP API Calls */
    ...
    while (1) {
        ...
        while (g_rx_q_head != g_rx_q_tail) {
            ...
            if (p != NULL) {
                ...
                g_netif.input(p, &g_netif); /* lwIP safely owns & frees pbuf */
            }
        ...
        if (get_cli_line(cli_input, sizeof(cli_input))) {
            ...
            if (strncmp(cmd, "arp ", 4) == 0) {
                ...
                PRINT_STRING("Unknown command. Example: 'arp 192.168.2.197' or 'status'.\r\n");
            }
