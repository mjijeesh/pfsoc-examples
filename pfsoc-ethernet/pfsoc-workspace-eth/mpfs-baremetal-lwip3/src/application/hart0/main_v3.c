 ...
 * Static IP: 192.168.2.207 | DHCP | ICMP Ping | Embedded HTTP Server
 *******************************************************************************/
...
/* Embedded File System Implementation for HTTPD */
static const char g_html_page[] =
    "<html><head><title>PolarFire SoC</title></head>"
    "<body><h1>PolarFire SoC Bare-Metal lwIP Web Server</h1>"
    "<p>Hardware GEM0 Ethernet Engine is Operational!</p></body></html>";
...
static void lwip_network_init(void) {
    ...
    PRINT_STRING("[lwIP] Complete Network Engine Initialized.\r\n");
}
...
void mac_task(void *pvParameters) {
    ...
    PRINT_STRING("\r\n=======================================================\r\n");
    PRINT_STRING(" PolarFire SoC - Complete lwIP Network Engine\r\n");
    PRINT_STRING("=======================================================\r\n");
    __enable_irq();
    ...
    while (1) {
        ...
        if (get_cli_line(cli_input, sizeof(cli_input))) {
            ...
            if (strncmp(cmd, "arp ", 4) == 0) {
                ...
                PRINT_STRING("[DHCP] Requesting IP address from network...\r\n");
                dhcp_stop(&g_netif);
