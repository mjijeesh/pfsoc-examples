...
Platform with the Peripheral Daughter Board, the Icicle Kit, the
BeagleV-Fire and the Discovery Kit.
...
The MSS_MAC_HW_PLATFORM macro definition in the
mss_ethernet_mac_sw_cfg.h identifies the hardware
platform and interface modes to use.
We just need to add Defined symbol in C Compiler from project properties the Target board that we are working on:
TARGET_DISCOVERY_KIT - PFSoC Discovery Kit
TARGET_ICICLE_KIT - PFSoC Icicle Kit
TARGET_BEAGLEV_FIRE - BeagleV Fire Kit
...
Valid values are:
  ...
  MSS_MAC_DESIGN_DISCOVERY_GEM0 - Discovery Kit GEM0
...
most of the options.
Note: The list of commands may vary based on the target board.
...
~~~
    ...
    J46    Closed
~~~
## Discovery Kit design has the following setup for the VSC8221:

  NRESET is connected to pin A14 of the G5SoC - GPIO127NB1.
  NSRESET is connected to pin E20 of the G5SoC - GPIO15NB1.

  CMODE 0 to 2 are connected with resistors to ground or VCC. They also
  connect to the G5SoC but the use of resistors means they cannot
  be changed via the I/O. The current default strapping is:

~~~
    CMODE 0 - 5.9K to VCC (A5)
    CMODE 1 - 4.02K to VCC (B5)
    CMODE 2 - 22.6K to VCC (C5)
    CMODE 2 - 0R to VCC (B4)
~~~
...
~~~
  Sigdet 0 is connected to A17 - GPIO1PB1
  EECLK is connected to AT24CM01 EEPROM SCL pin
  EEDAT is connected to AT24CM01 EEPROM SDA pin
~~~

~~~
  RXDP is connected to K8
  RXDN is connected to K7
  TXDP is connected to K9
  TXDN is connected to K10
~~~

~~~
  MDINTB is connected to A13
  MDC is connected to J10
  MDIO is connected to J9
~~~

~~~
  CLKOUT is connected to D21 - GPIO11PB1/CLKIN_S_9
~~~
