...
static void set_board_mac_address(uint8_t m1, uint8_t m2, uint8_t m3, uint8_t m4, uint8_t m5, uint8_t m6) {
    ...
    /* Write new MAC address directly to GEM0 Station Address 1 registers (0x080 / 0x084) */
...
void mac_task(void *pvParameters) {
    ...
    while (1) {
        ...
        if (get_cli_line(cli_input, sizeof(cli_input))) {
            ...
            else if (strncmp(cmd, "set ip ", 7) == 0 || strncmp(cmd, "ip set ", 7) == 0) {
                uint32_t ip1 = 0, ip2 = 0, ip3 = 0, ip4 = 0;
                char *arg = (strncmp(cmd, "set ip ", 7) == 0) ? (cmd + 7) : (cmd + 7);
                if (sscanf(arg, "%u.%u.%u.%u", &ip1, &ip2, &ip3, &ip4) == 4) {
                    set_board_ip_address((uint8_t)ip1, (uint8_t)ip2, (uint8_t)ip3, (uint8_t)ip4);
                } else {
                    PRINT_STRING("Usage: set ip <ip1>.<ip2>.<ip3>.<ip4>\r\n");
                }
            }
            else if (strncmp(cmd, "set mac ", 8) == 0 || strncmp(cmd, "set hw ", 7) == 0) {
                uint32_t m1 = 0, m2 = 0, m3 = 0, m4 = 0, m5 = 0, m6 = 0;
                char *arg = (strncmp(cmd, "set mac ", 8) == 0) ? (cmd + 8) : (cmd + 7);
                if (sscanf(arg, "%x:%x:%x:%x:%x:%x", &m1, &m2, &m3, &m4, &m5, &m6) == 6 ||
                    sscanf(arg, "%x-%x-%x-%x-%x-%x", &m1, &m2, &m3, &m4, &m5, &m6) == 6) {
