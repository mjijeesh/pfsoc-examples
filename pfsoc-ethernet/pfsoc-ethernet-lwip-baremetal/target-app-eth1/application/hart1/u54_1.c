...
#define DHCP_TIMEOUT_MS 10000U /* 10 Seconds DHCP Timeout Limit */
...
static uint8_t g_board_ip[4] = {192, 168, 20, 44};
...
static bool g_dhcp_timing_out = false;
static uint32_t g_dhcp_start_tick = 0;
static bool g_last_link_state = true;

struct netif g_netif;

/* SBSS and BSS memory symbols from linker script */
extern unsigned long __sbss_start;
extern unsigned long __bss_end;

static void zero_bss_section(void) {
    uint8_t *dst = (uint8_t *)&__sbss_start;
    uint8_t *end = (uint8_t *)&__bss_end;
    while (dst < end) {
        *dst++ = 0;
    }
}

/* Embedded Web Page (Including HTTP Response Headers & Explicit Content-Length) */
static const char g_html_page[] =
    "HTTP/1.0 200 OK\r\n"
    "Server: lwIP/2.1.2 (PolarFire SoC Bare-Metal)\r\n"
...
err_t fs_open(struct fs_file *file, const char *name) {
    if (!file || !name) return ERR_VAL;

    /* Serves HTML for root requests; returns 404 for favicon/other paths */
    if (strcmp(name, "/") != 0 && 
        strcmp(name, "/index.html") != 0 && 
        strcmp(name, "/index.htm") != 0) {
        return ERR_VAL;
    }
...
int fs_read(struct fs_file *file, char *buffer, int count) { 
    if (!file || !buffer || count <= 0) return FS_READ_EOF;
    int read_len = file->len - file->index;
    if (read_len > count) {
        read_len = count;
    }
    if (read_len <= 0) {
...
static void low_level_init(void) {
    ...
    g_dhcp_requested = false;
    g_dhcp_bound_reported = false;
    g_dhcp_timing_out = false;
    g_dhcp_start_tick = 0;
...
static void lwip_network_init(void) {
    ...
    /* Initialize interface with 0.0.0.0 for initial DHCP Discovery */
    ...
    /* Auto-start DHCP Client on startup */
    PRINT_STRING("[DHCP] Initiating Auto-DHCP Discovery (10s Timeout limit)...\r\n");
    g_dhcp_requested = true;
    g_dhcp_bound_reported = false;
    g_dhcp_timing_out = true;
    g_dhcp_start_tick = g_tick_counter;
    dhcp_start(&g_netif);
}
...
static void send_lwip_arp_request(uint8_t ip1, uint8_t ip2, uint8_t ip3, uint8_t ip4) {
    ...
    char dbg[128];
    sprintf(dbg, "\r\n[DEBUG] Transmitting Manual ARP Request for %u.%u.%u.%u...\r\n", ip1, ip2, ip3, ip4);
    PRINT_STRING(dbg);

    if (!netif_is_up(&g_netif) || !netif_is_link_up(&g_netif)) {
        PRINT_STRING("[ERROR] Network Interface is DOWN or Link Disconnected.\r\n");
        return;
    }

    uint32_t avail_tx = g_test_mac->queue[0].nb_available_tx_desc;
    sprintf(dbg, "[DEBUG] Free GEM TX Descriptors: %u / %u\r\n", (unsigned int)avail_tx, TX_RING_SLOTS);
    PRINT_STRING(dbg);

    if (avail_tx == 0) {
        PRINT_STRING("[ERROR] MAC TX Queue Full!\r\n");
        return;
    }

    err_t err = etharp_request(&g_netif, &target_ip);
    sprintf(dbg, "[DEBUG] etharp_request() returned: %d (%s)\r\n", 
            (int)err, (err == ERR_OK) ? "ERR_OK" : "ERR_FAILED");
    PRINT_STRING(dbg);
}
...
static u8_t ping_recv_cb(void *arg, struct raw_pcb *pcb, struct pbuf *p, const ip_addr_t *addr) {
    (void)arg; (void)pcb;
    if (p != NULL && p->tot_len >= (20 + sizeof(struct icmp_echo_hdr))) {
...
static void send_ping(uint8_t ip1, uint8_t ip2, uint8_t ip3, uint8_t ip4) {
    ...
    if (!ping_pcb) {
        ...
        if (ping_pcb != NULL) {
            ...
            PRINT_STRING("[ERROR] Failed to allocate raw ICMP PCB.\r\n");
            return;
        }
    }
    ...
    if (!p) {
        PRINT_STRING("[ERROR] pbuf_alloc failed for Ping Request.\r\n");
        return;
    }
...
void mac_task(void *pvParameters) {
    ...
    while (1) {
        ...
        /* Asynchronous DHCP Lease & Timeout Monitor */
        if (g_dhcp_requested) {
            if (dhcp_supplied_address(&g_netif)) {
                if (!g_dhcp_bound_reported) {
            ...
            } else if (g_dhcp_timing_out && ((g_tick_counter - g_dhcp_start_tick) >= DHCP_TIMEOUT_MS)) {
                g_dhcp_timing_out = false;
                g_dhcp_requested = false;
                dhcp_stop(&g_netif);

                ip4_addr_t static_ip, static_nm, static_gw;
                IP4_ADDR(&static_ip, g_board_ip[0], g_board_ip[1], g_board_ip[2], g_board_ip[3]);
                IP4_ADDR(&static_nm, 255, 255, 255, 0);
                IP4_ADDR(&static_gw, g_board_ip[0], g_board_ip[1], g_board_ip[2], 1);
                netif_set_addr(&g_netif, &static_ip, &static_nm, &static_gw);

                sprintf(dbg_buf, "\r\n[DHCP TIMEOUT] No DHCP Server response within 10s. Fallback to Static IP!\r\n"
                                 "  Assigned Static IP : %u.%u.%u.%u\r\n"
                                 "  Subnet Mask         : 255.255.255.0\r\n"
                                 "  Gateway             : %u.%u.%u.1\r\neth-cli> ",
                                 g_board_ip[0], g_board_ip[1], g_board_ip[2], g_board_ip[3],
                                 g_board_ip[0], g_board_ip[1], g_board_ip[2]);
                PRINT_STRING(dbg_buf);
            }
        }
        ...
        while (g_rx_q_head != g_rx_q_tail) {
            ...
            if (p != NULL) {
                ...
                /* Real-time Event Inspector for Ping, HTTP, and iPerf */
                if (len >= 38) {
                    uint16_t ethertype = (data[12] << 8) | data[13];
                    if (ethertype == 0x0800) { /* IPv4 */
                        uint8_t proto = data[23];
                        if (proto == 1 && len >= 38 && data[34] == 8) { /* ICMP Echo Request */
                            sprintf(dbg_buf, "\r\n[PING EVENT] Inbound Ping Request received from %u.%u.%u.%u\r\neth-cli> ",
                                    data[26], data[27], data[28], data[29]);
                            PRINT_STRING(dbg_buf);
                        } else if (proto == 6) { /* TCP */
                            uint16_t dport = (data[36] << 8) | data[37];
                            if (dport == 80 && (data[47] & 0x02)) { /* SYN on Port 80 */
                                sprintf(dbg_buf, "\r\n[HTTP EVENT] Web server page accessed by %u.%u.%u.%u\r\neth-cli> ",
                                        data[26], data[27], data[28], data[29]);
                                PRINT_STRING(dbg_buf);
                            } else if (dport == 5001 && (data[47] & 0x02)) { /* SYN on Port 5001 */
                                sprintf(dbg_buf, "\r\n[IPERF EVENT] Connection request on Port 5001 from %u.%u.%u.%u\r\neth-cli> ",
                                        data[26], data[27], data[28], data[29]);
                                PRINT_STRING(dbg_buf);
                            }
                        }
