/*******************************************************************************
 * PolarFire SoC Discovery Kit - Ethernet Diagnostic Suite
...
/* Background handler to auto-respond to incoming ARP & ICMP Echo Requests */
...
static void process_auto_responses(mss_mac_instance_t *mac, uint8_t *pkt, uint32_t len) {
    ...
    /* 1. Respond to incoming ARP Requests directed to g_board_ip */
    if (ethertype == 0x0806) {
        uint16_t opcode = (uint16_t)(((uint16_t)pkt[20] << 8) | pkt[21]);
        if (opcode == 1 && memcmp(&pkt[38], g_board_ip, 4) == 0) {
            uint8_t reply[60] = {0};
            memcpy(&reply[0], &pkt[6], 6);            /* Dest MAC = Sender MAC */
            memcpy(&reply[6], mac->mac_addr, 6);       /* Src MAC = Board MAC */
            ...
            reply[20] = 0x00; reply[21] = 0x02;        /* Opcode: ARP Reply */
            memcpy(&reply[22], mac->mac_addr, 6);      /* Sender MAC */
            memcpy(&reply[28], g_board_ip, 4);         /* Sender IP */
            memcpy(&reply[32], &pkt[6], 6);            /* Target MAC */
            memcpy(&reply[38], &pkt[28], 4);           /* Target IP */

            MSS_MAC_send_pkt(mac, 0, reply, 60, NULL);
        }
    }
    /* 2. Respond to incoming ICMP Echo Requests directed to g_board_ip */
        ...
        if (protocol == 1 && memcmp(&pkt[30], g_board_ip, 4) == 0) {
            ...
            if (icmp_type == 8) { /* Echo Request */
                ...
                /* ICMP Type 0 (Echo Reply) & Checksum */
                reply[34] = 0;
                ...
                /* Print notification on UART terminal */
                char info[128];
...
static void mac_rx_callback(void *this_mac, uint32_t queue_no, uint8_t *p_rx_packet, uint32_t pckt_length, mss_mac_rx_desc_t *cdesc, void *caller_info) {
    ...
    /* Background auto-responder for ARP & Ping requests */
...
void mac_task(void *pvParameters) {
    ...
    PRINT_STRING("\r\n***** PolarFire SoC Discovery Kit Diagnostic Suite *****\r\n");
    __enable_irq();
