/*******************************************************************************
 * Network Subsystem: Driver Initialization & Network Tasks
 *******************************************************************************/
...
void low_level_init(void) {
    log_msg("[DRIVER] Initializing MSS MAC configuration structures...\r\n");
    ...
    log_msg("[DRIVER] Executing MSS_MAC_init() and PHY Auto-negotiation...\r\n");
    MSS_MAC_init(g_test_mac, &g_mac_config);
    log_msg("[DRIVER] GEM0 MAC and VSC8221 SGMII PHY initialized successfully.\r\n");

    log_msg("[DRIVER] Registering TX and RX interrupt callbacks...\r\n");
    MSS_MAC_set_tx_callback(g_test_mac, 0, packet_tx_complete_handler);
    MSS_MAC_set_rx_callback(g_test_mac, 0, mac_rx_callback);

    log_msg("[DRIVER] Populating DMA receive ring buffers...\r\n");
    ...
    log_msg("[DRIVER] Low-level GEM0 DMA hardware setup complete.\r\n");
}
...
void tcpip_init_done_cb(void *arg) {
    ...
    log_msg("[STACK] lwIP tcpip_thread initialized. Registering netif...\r\n");
    ...
    log_msg("[SERVICES] Starting HTTP Web Dashboard (Port 80)...\r\n");
    httpd_init();

    log_msg("[SERVICES] Starting iPerf Bandwidth Server (Port 5001)...\r\n");
    lwiperf_start_tcp_server_default(my_iperf_report_cb, NULL);

    /* FEATURE 2: Auto-start DHCP Discovery with Fallback */
    log_msg("[DHCP] Starting DHCP Client (10-second timeout to Static Fallback)...\r\n");
    g_dhcp_requested = true;
    g_dhcp_bound_reported = false;
    g_dhcp_fallback_active = false;
    dhcp_start(&g_netif);

    log_msg("[DEBUG] Network engine running. Stack and services active.\r\n");
    print_status();
}
    ...
    while (1) {
        if (xSemaphoreTake(g_rx_sem, portMAX_DELAY) == pdTRUE) {
            while (g_rx_q_head != g_rx_q_tail) {
                ...
                /* FEATURE 1: Packet Inspector Streams to BOTH Terminal and Web Dashboard */
                if (g_debug_monitor && len >= 14) {
                    ...
                    if (ethertype == 0x0806 && len >= 28) { /* ARP */
                        ...
                        if (op == 1 && data[38] == g_board_ip[0] && data[39] == g_board_ip[1]) {
                            sprintf(dbg_buf, "\r\n[MONITOR] RX ARP Request for %u.%u.%u.%u\r\neth-cli> ", data[38], data[39], data[40], data[41]);
                            log_msg(dbg_buf);
                        }
                    } else if (ethertype == 0x0800 && len >= 34) { /* IPv4 */
                        uint8_t proto = data[23];
                        if (proto == 1 && len >= 38 && data[34] == 8) { /* ICMP Echo */
                            sprintf(dbg_buf, "\r\n[MONITOR] RX Ping Request from %u.%u.%u.%u\r\neth-cli> ", data[26], data[27], data[28], data[29]);
                            log_msg(dbg_buf);
                        } else if (proto == 6 && len >= 36) { /* TCP */
                            uint16_t dport = (data[36] << 8) | data[37];
                            if (dport == 80 && (data[47] & 0x02)) {
                                sprintf(dbg_buf, "\r\n[MONITOR] HTTP GET Request from %u.%u.%u.%u\r\neth-cli> ", data[26], data[27], data[28], data[29]);
                                log_msg(dbg_buf);
                            }
                        }
                    }
...
void phy_monitor_task(void *pvParameters) {
    ...
    while (1) {
        ...
        /* FEATURE 2: DHCP Timeout Monitor & Static Fallback Logic */
        if (g_dhcp_requested && !g_dhcp_bound_reported) {
            if (dhcp_supplied_address(&g_netif)) {
                g_dhcp_bound_reported = true;
                g_dhcp_fallback_active = false;
                sprintf(dbg_buf, "\r\n[DHCP SUCCESS] Dynamic IP Assigned: %s\r\neth-cli> ", ipaddr_ntoa(&g_netif.ip_addr));
                log_msg(dbg_buf);
            } else if (!g_dhcp_fallback_active) {
                dhcp_timer_sec++;
                if (dhcp_timer_sec >= 10) { /* 10 Seconds Fallback Timeout */
                    g_dhcp_fallback_active = true;
                    dhcp_stop(&g_netif);
                    ...
                    sprintf(dbg_buf, "\r\n[DHCP TIMEOUT] No DHCP Server found within 10s. Fallback to Static IP: %u.%u.%u.%u\r\neth-cli> ",
