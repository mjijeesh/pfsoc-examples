 ...
 * Features:
 *  - Real-Time Terminal Log Stream Window (Auto-Scrolling)
 *  - Auto-DHCP & Static Fallback Status Indicator
 *  - FreeRTOS Task List, Execution State & Memory High-Water Mark
 *  - Interactive Traffic Monitor Toggle Buttons
 *******************************************************************************/
...
/* Expanded to 12 KB to prevent HTML truncation across all tasks */
static char g_html_buf[12288];
...
err_t fs_open(struct fs_file *file, const char *name) {
    ...
    const char *dhcp_badge = "DISABLED (Static)";
    if (dhcp_supplied_address(&g_netif)) {
        dhcp_badge = "<span style='color:#4ade80;'>BOUND (Dynamic IP)</span>";
    } else if (g_dhcp_fallback_active) {
        dhcp_badge = "<span style='color:#facc15;'>FALLBACK (Static IP 192.168.20.207)</span>";
    } else if (g_dhcp_requested) {
        dhcp_badge = "<span style='color:#38bdf8;'>DISCOVERING...</span>";
    }

    uint32_t uptime_sec = (uint32_t)(xTaskGetTickCount() / configTICK_RATE_HZ);

    /* Construct Web Dashboard */
    int len = snprintf(g_html_buf, sizeof(g_html_buf),
        ...
        "th{text-align:left;padding:6px 8px;color:#94a3b8;font-size:12px;border-bottom:1px solid #334155;}"
        "td{padding:8px;border-bottom:1px solid #334155;font-size:13px;}"
        ...
        ".terminal{background:#020617;border:1px solid #334155;border-radius:6px;padding:12px;font-family:Consolas,monospace;font-size:12px;color:#4ade80;height:160px;overflow-y:auto;white-space:pre-wrap;}"
        ".footer{margin-top:20px;font-size:12px;color:#64748b;text-align:center;}"
        "</style></head><body>"
        "<h1>PolarFire SoC Real-Time Network & Kernel Dashboard</h1>"
        "<div class='grid'>"

        /* Card 1: Network & MAC */
        "<div class='card'><h2>MAC & Network Layer</h2><table>"
        "<tr><td>Interface</td><td style='text-align:right;font-weight:600;color:#38bdf8;'>%c%c</td></tr>"
        "<tr><td>MAC Address</td><td style='text-align:right;font-weight:600;color:#38bdf8;'>%02X:%02X:%02X:%02X:%02X:%02X</td></tr>"
        "<tr><td>IPv4 Address</td><td style='text-align:right;font-weight:600;color:#38bdf8;'>%s</td></tr>"
        "<tr><td>Subnet Mask</td><td style='text-align:right;font-weight:600;color:#38bdf8;'>%s</td></tr>"
        "<tr><td>Gateway</td><td style='text-align:right;font-weight:600;color:#38bdf8;'>%s</td></tr>"
        "<tr><td>DHCP Engine</td><td style='text-align:right;font-weight:600;'>%s</td></tr>"
        "</table></div>"

        /* Card 2: Hardware PHY Transceiver */
        "<div class='card'><h2>PHY Hardware Status</h2><table>"
        "<tr><td>Transceiver</td><td style='text-align:right;font-weight:600;color:#38bdf8;'>VSC8221 (MDIO 11)</td></tr>"
        "<tr><td>Link State</td><td style='text-align:right;'><span class='%s'>%s</span></td></tr>"
        "<tr><td>Negotiated Speed</td><td style='text-align:right;font-weight:600;color:#38bdf8;'>%s</td></tr>"
        "<tr><td>Duplex Mode</td><td style='text-align:right;font-weight:600;color:#38bdf8;'>%s</td></tr>"
        "</table></div>"

        /* Card 3: Network Traffic & Traffic Inspector Control */
        "<div class='card'><h2>Traffic & Live Inspector</h2><table>"
        "<tr><td>Packets Transmitted (Tx)</td><td style='text-align:right;font-weight:600;color:#38bdf8;'>%lu frames</td></tr>"
        "<tr><td>Packets Received (Rx)</td><td style='text-align:right;font-weight:600;color:#38bdf8;'>%lu frames</td></tr>"
        "<tr><td>CLI Packet Inspector</td><td style='text-align:right;font-weight:600;'>%s</td></tr>"
        "<tr><td>Toggle Inspector</td><td style='text-align:right;'>"
        ...
        "<tr><td>System Uptime</td><td style='text-align:right;font-weight:600;color:#38bdf8;'>%lu seconds</td></tr>"
        "<tr><td>Total Active Tasks</td><td style='text-align:right;font-weight:600;color:#38bdf8;'>%u tasks</td></tr>"
        "<tr><td>Current Free Heap</td><td style='text-align:right;font-weight:600;color:#38bdf8;'>%u bytes</td></tr>"
        "<tr><td>Min Ever Free Heap</td><td style='text-align:right;font-weight:600;color:#38bdf8;'>%u bytes</td></tr>"
        "</table></div>",
    ...
    /* Live Auto-Scrolling Terminal Stream Window */
    len += snprintf(g_html_buf + len, sizeof(g_html_buf) - len,
        "<div class='card' style='grid-column:1/-1;'><h2>Live System & Packet Inspector Console Output</h2>"
        "<div class='terminal' id='term_win'>%s</div>"
        "</div>"
        "<script>var w=document.getElementById('term_win');if(w){w.scrollTop=w.scrollHeight;}</script>",
        strlen(g_web_log_buf) > 0 ? g_web_log_buf : "System initialized. Waiting for traffic..."
    );

    /* FreeRTOS Task Breakdown Table */
    len += snprintf(g_html_buf + len, sizeof(g_html_buf) - len,
    ...
    if (task_array != NULL) {
        ...
        for (UBaseType_t i = 0; i < task_count; i++) {
            ...
            len += snprintf(g_html_buf + len, sizeof(g_html_buf) - len,
                ...
                "<td>%u</td>"
                "<td>#%u</td>"
                "<td style='color:#38bdf8;font-weight:600;'>%u bytes free</td>"
                "</tr>",
