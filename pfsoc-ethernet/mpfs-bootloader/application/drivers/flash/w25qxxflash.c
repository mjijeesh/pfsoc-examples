...
/**
 * @brief Polls Winbond Status Register 1 until BUSY bit (Bit 0) clears.
 */
...
void W25QXX_FLASH_read(uint32_t address, uint8_t *rx_buffer, size_t size_in_bytes)
{
    ...
    while (bytes_read < size_in_bytes)
    {
        ...
        uint16_t header_len = prepare_address_header(W25Q_CMD_READ_DATA, current_addr, cmd_buffer);

        hal_spi_select();
        hal_spi_transfer(cmd_buffer, header_len, &rx_buffer[bytes_read], (uint16_t)chunk_size);
        hal_spi_deselect();
