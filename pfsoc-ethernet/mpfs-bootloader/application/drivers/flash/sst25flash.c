...
void SST25_FLASH_read(uint32_t address, uint8_t *rx_buffer, size_t size_in_bytes)
{
    ...
    while (bytes_read < size_in_bytes)
    {
        ...
        cmd_buffer[1] = (uint8_t)((current_addr >> 16) & 0xFF);
        cmd_buffer[2] = (uint8_t)((current_addr >> 8) & 0xFF);
        cmd_buffer[3] = (uint8_t)(current_addr & 0xFF);

        hal_spi_select();
        hal_spi_transfer(cmd_buffer, 4, &rx_buffer[bytes_read], (uint16_t)chunk_size);
        hal_spi_deselect();
