...
/* Normally defined in board_config.h */
...
void autoboot_run(int timeout_sec)
{
    ...
    printf("\r--- Attempting Boot Method 1: Serialboot (SFL) ---\n");
    if (serialboot() == 0) {
        return;
    }
...
void boot(unsigned long r1, unsigned long r2, unsigned long r3, unsigned long addr)
{
    ...
    printf("\r\nLaunching target application on U54 harts at 0x%016lx...\n", addr);
...
int serialboot(void)
{
    ...
    if (ack_status == ACK_TIMEOUT) {
        printf("\r\nTimeout waiting for SFL host connection.\n");
        return 1;
    }
    if (ack_status == ACK_CANCELLED) {
        printf("\r\nSerial boot cancelled by user.\n");
        return 0;
    }
