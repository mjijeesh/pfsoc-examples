...
{
    uint32_t flash_offset = SPI_FLASH_BOOT_ADDR ;  /* Default  Flash Offset */
    uintptr_t ram_addr    = PAYLOAD_RAM_ADDR;   /* Default DDR or LIM */
    size_t max_ram_size = 0;
    uint8_t header[8];
    uint8_t mfr_id = 0, dev_id = 0,capacity_id = 0;
    ...
    spi_flash_init();
    spi_flash_read_id(&mfr_id, &dev_id,&capacity_id);
    printf("Active Driver : %s\n", spi_flash_get_driver_name());
    printf("Flash Hardware: Mfr=0x%02X, Dev=0x%02X,Capacity=0x%02X\n", mfr_id, dev_id,capacity_id);
    ...
    if (image_size == 0 || image_size == 0xFFFFFFFF) {
        printf("Error: Invalid FBI image size (0x%08X) at Flash 0x%08X! Is flash erased?\n",
               (unsigned int)image_size, (unsigned int)flash_offset);
        return;
    }
    ...
    if (image_size > max_ram_size) {
        printf("Error: Image size (%u bytes) exceeds RAM region capacity (%u bytes)!\n",
               (unsigned int)image_size, (unsigned int)max_ram_size);
        return;
    }

    printf("Copying %u bytes from Flash 0x%08X to RAM 0x%016lx...\n",
           (unsigned int)image_size, (unsigned int)(flash_offset + 8), (unsigned long)ram_addr);
    ...
    if (calculated_crc != expected_crc) {
        printf("Error: CRC32 mismatch! Header=0x%08X, Computed=0x%08X\n",
               (unsigned int)expected_crc, (unsigned int)calculated_crc);
        return;
    }
...
void flash_write_handler(int nb_params, char **params)
{
    ...
    spi_flash_init();
    printf("Writing %u bytes from RAM 0x%016lx to Flash 0x%08X (%s)...\n",
           (unsigned int)count, (unsigned long)ram_addr, (unsigned int)flash_offset, spi_flash_get_driver_name());
...
void flash_copy_handler(int nb_params, char **params)
{
    ...
    spi_flash_init();
    printf("Copying %u bytes from Flash 0x%08X to RAM 0x%016lx (%s)...\n",
           (unsigned int)count, (unsigned int)flash_offset, (unsigned long)ram_addr, spi_flash_get_driver_name());
...
void flash_read_handler(int nb_params, char **params)
{
    ...
    spi_flash_init();
    printf("Reading %u bytes from Flash Address 0x%08X:\n", (unsigned int)requested_len, (unsigned int)flash_offset);
...
void flash_erase_range_handler(int nb_params, char **params)
{
    ...
    spi_flash_init();
    printf("Erasing Flash region 0x%08X (+%u bytes)...\n", (unsigned int)offset, (unsigned int)count);
...
void flash_idcode_handler(int nb_params, char **params)
{
   ...
   if (spi_flash_read_id(&mfr_id, &dev_id, &capacity_id) == 0) {
