 ...
 void print_system_info(void)
{
    ...
    printf("\r\n SPI Interface    : FPGA CoreSPI IP (Base: 0x%08X)", (unsigned int)CORESPI_BASE_ADDR);
#elif defined(USE_MSS_SPI1)
    printf("\r\n SPI Interface    : Hard MSS SPI1 (Base: 0x20101000)");
#else
    printf("\r\n SPI Interface    : Hard MSS SPI0 (Base: 0x20100000)");
#endif
    printf("\r\n Active SPI Flash : %s", spi_flash_get_driver_name());   
...
/* 1. Initialize UART for console output */
    ...
    /* 2. Initialize SPI Flash subsystem */
    spi_flash_init();

    /* 3. Print the system banner */
    print_system_info();

    /* 4. Launch BIOS Shell / Autoboot */
    autoboot_run(10);

    /* 5. Interactive CLI Shell Loop */
    cli_loop();
