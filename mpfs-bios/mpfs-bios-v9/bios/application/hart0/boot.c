...
#define CPU_FREQ_HZ  600000000UL // E51 Core Clock (600 MHz)
...
// Helper to read RISC-V Machine Cycle counter
...
// Memory boundary validation for PolarFire SoC
...
/* only Hart 0 should be enabled i nthe taregt app here, all other harts are already in wfi */
...
void __attribute__((noreturn)) bootold(unsigned long r1, unsigned long r2, unsigned long r3, unsigned long addr)
{
    ...
    // 1. Data memory fence: flush all pending write buffers to LIM memory
...
/* New boot function that boots all harts (0..4) to the same address. The first hart (0) will jump to the address,
    and the other harts will be woken up via IPI and will also jump to the same address. */

// Global jump address shared across harts
...
void boot(unsigned long r1, unsigned long r2, unsigned long r3, unsigned long addr)
{
    ...
    // 1. Publish jump address for U54 harts
    g_app_jump_addr = addr;
    asm volatile("fence" ::: "memory");

    // 2. Raise IPIs to U54 cores (Hart 1 through 4)
    for (uint32_t hart_id = 1; hart_id <= 4; hart_id++) {
        raise_soft_interrupt(hart_id);
    }

    // 3. Delay to allow U54 cores to unblock and jump
    for (volatile int i = 0; i < 200000; i++);

    asm volatile("fence.i" ::: "memory");

    // Hart 0 returns cleanly to serialboot() -> litex> CLI loop
}
...
static int check_ack(void)
{
    ...
    uint64_t timeout_cycles = (CPU_FREQ_HZ / 4); // 250ms timeout
...
int serialboot(void)
{
    ...
    // Send SFL Magic Request sequence to host terminal
    c = str;
    while (*c) {
        uart_write(*c++);
    }
    ...
    // Magic ACK matched, proceed with receiving frames
    while (1) {
        int i = 0;
        int timeout = 1;
        uint64_t frame_timeout_cycles = (CPU_FREQ_HZ / 4); // 250ms per byte
        uint64_t start_cycles = get_cycles();

        while ((get_cycles() - start_cycles) < frame_timeout_cycles) {
            if (uart_read_nonblock()) {
                unsigned char data = (unsigned char)uart_read();
                start_cycles = get_cycles(); // Re-arm timer on byte receipt
        ...
        case SFL_CMD_JUMP: {
            ...
            // Return cleanly from serialboot() back to the CLI prompt
            return 0;
